...
void Product2_Series_Rep::DAC(mpq_t *f, mpq_t *g, int indSeriesSize, 
	...
	while (preProductSize < 2 * indSeriesSize - 1) {
		...
		//if (depth > 7) {
			// Serial execution for deep recursions to limit overhead
		...
		//} else {
			...
			std::future<void> highFuture = pool.submitTask([&] {
				DAC(f_up_star, g_up_star, hiArray, (preProductSize + 1) / 2, pool, depth + 1);
			});
			std::future<void> midFuture = pool.submitTask([&] {
				DAC(f_low_plus_up, g_low_plus_up, midArray, (preProductSize + 1) / 2, pool, depth + 1);
			});

			while ((highFuture.wait_for(std::chrono::seconds(0)) ==
				std::future_status::timeout) ||
				(midFuture.wait_for(std::chrono::seconds(0)) ==
				std::future_status::timeout)) {
				pool.runPendingTask();
			}
...
void Product2_Series_Rep::DAC(mpq_t *f, mpq_t *g, mpq_t *product, int indSeriesSize, 
	...
	if (indSeriesSize <= DnCThreshold) {
		...
		if (depth > 7) {
			...
			// Parallel execution for top levels
			// std::cout << "Depth is: " << depth << std::endl;
			std::future<void> lowFuture = pool.submitTask([&] {
				DAC(f_low_star, g_low_star, loArray, indSeriesSize / 2, pool, depth + 1);
			});
			std::future<void> highFuture = pool.submitTask([&] {
				DAC(f_up_star, g_up_star, hiArray, indSeriesSize / 2, pool, depth + 1);
			});
			std::future<void> midFuture = pool.submitTask([&] {
				DAC(f_low_plus_up, g_low_plus_up, midArray, indSeriesSize / 2, pool, depth + 1);
			});

			while ((lowFuture.wait_for(std::chrono::seconds(0)) ==
				std::future_status::timeout) ||
				(highFuture.wait_for(std::chrono::seconds(0)) ==
				std::future_status::timeout) ||
				(midFuture.wait_for(std::chrono::seconds(0)) ==
				std::future_status::timeout)) {
				pool.runPendingTask();
